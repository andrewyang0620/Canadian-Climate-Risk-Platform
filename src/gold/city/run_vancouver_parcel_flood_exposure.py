...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build one-row-per-parcel Vancouver flood "
            "exposure Gold."
        )
    )
...
def run_vancouver_parcel_flood_exposure(
    *,
    ...
    scenario_path = latest_table_parquet(
        root=gold_root,
        table_name=(
    ...
    print(
        "[OK] wrote Vancouver parcel flood exposure | "
        f"rows={summary['output_row_count']} "
        f"exposed={summary['exposed_parcel_count']} "
        f"exposure_rate="
        f"{summary['parcel_exposure_rate']:.4%} "
        f"multi_scenario="
        f"{summary['multi_scenario_parcel_count']} "
        f"run_id={run_id}"
    )
...
def main() -> None:
    ...
    result = run_vancouver_parcel_flood_exposure(
