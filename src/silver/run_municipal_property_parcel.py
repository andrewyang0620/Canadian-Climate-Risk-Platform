...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Silver municipal property parcel outputs.")
...
def main() -> None:
    ...
    raw_path = latest_successful_bronze_raw_path(source_name="vancouver_property_parcels")

    dataframe = build_vancouver_property_parcel_silver(raw_path)
    ...
    output_path = output_dir / "silver_property_parcel.parquet"
    dataframe.to_parquet(output_path, index=False)

    metadata_dir = (
        Path(args.silver_root)
        / "_metadata"
        / "municipal_property_parcel"
        / f"extract_date={args.extract_date}"
        / f"run_id={run_id}"
    )
    ...
    print(
        "[OK] wrote municipal property parcel Silver outputs | "
